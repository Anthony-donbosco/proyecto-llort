...
if (!isset($_POST['action']) || !isset($_POST['partido_id'])) {
    header("Location: gestionar_amistosos.php?error=Parámetros inválidos.");
    exit;
}
...
$redirect_url = "editar_amistoso.php?partido_id=$partido_id";
$redirect_partidos = "gestionar_amistosos.php"; 

if ($action == 'editar_amistoso') { 
    try {
        ...
        $sql .= " WHERE id = ? AND torneo_id IS NULL"; 
        $params[] = $partido_id;
        $types .= "i";
        ...
        if ($stmt->affected_rows >= 0) {
            $stmt->close();
            header("Location: $redirect_partidos");
        } else {
            $stmt->close();
    ...
    header("Location: $redirect_partidos?error=Acción no válida.");
}
